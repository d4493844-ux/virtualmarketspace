...
export default function VideoFeed({ mode }: VideoFeedProps) {
  ...
  const loadVideos = async () => {
    let query = supabase
      ...
      .neq('type', 'text'); // Text posts go to Status, not the feed
      ...
      {videos.map((video) => (
        ...
        >
          ...
          <div style={{ position: 'absolute', right: 16, bottom: 120, display: 'flex', flexDirection: 'column', gap: 20, zIndex: 10, alignItems: 'center' }}>
            ...
            <button
              onClick={() => navigate(`/status/${video.user_id}`)}
              style={{ position: 'relative', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}
            >
              {/* Gradient ring — indicates this user has a status */}
              <div style={{
                ...
                <img
                  ...
                  alt=""
                  style={{ width: 46, height: 46, borderRadius: '50%', objectFit: 'cover', border: '2px solid #000' }}
                />
              </div>
            </button>
            ...
            <button onClick={() => handleLike(video)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, background: 'none', border: 'none', cursor: 'pointer' }}>
              <Heart
                style={{ width: 24, height: 24, filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.5))', color: likedVideos.has(video.id) ? '#ef4444' : 'white' }}
            ...
            >
              <MessageCircle style={{ width: 24, height: 24, color: 'white', filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.5))' }} />
            ...
            <button onClick={() => handleShare(video)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, background: 'none', border: 'none', cursor: 'pointer' }}>
              <Share2 style={{ width: 24, height: 24, color: 'white', filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.5))' }} />
            ...
            <button onClick={() => handleBookmark(video)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, background: 'none', border: 'none', cursor: 'pointer' }}>
              <Bookmark
                style={{ width: 22, height: 22, filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.5))', color: bookmarkedVideos.has(video.id) ? '#f59e0b' : 'white' }}
            ...
            {video.product_tags && video.product_tags.length > 0 && (
              <button
                ...
                style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, background: 'none', border: 'none', cursor: 'pointer' }}
              >
                <ShoppingBag style={{ width: 22, height: 22, color: 'white', filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.5))' }} />
          ...
          {/* Comments drawer — fixed to viewport so BottomNav doesn't cover it */}
          {showComments === video.id && (
            <div
              style={{
                ...
                zIndex: 200,  // above BottomNav (z-index 100)
              }}
            ...
            >
              ...
              >
                <div style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid var(--border-color)', flexShrink: 0 }}>
                ...
                <div style={{ padding: '12px 16px 16px', borderTop: '1px solid var(--border-color)', display: 'flex', gap: 8, flexShrink: 0 }}>
                  <input
                    type="text"
                    value={commentText}
