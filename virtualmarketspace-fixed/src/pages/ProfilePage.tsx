...
export default function ProfilePage() {
  ...
  // FIX 6: split effects so other user profile loads content after profile state is set
  useEffect(() => {
  ...
  const loadUserContent = async (profileId: string) => {
  ...
  return (
    <div className="min-h-screen pb-20" style={{ backgroundColor: 'var(--bg-primary)' }}>
      <div className="sticky top-0 z-10 flex items-center justify-between p-4" style={{ backgroundColor: 'var(--bg-primary)', borderBottom: '1px solid var(--border-color)' }}>
        <h1 className="text-lg font-bold" style={{ color: 'var(--text-primary)' }}>Profile</h1>
        {isOwnProfile && (
        ...
        <div className="flex items-start gap-4 mb-6">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <h2 className="text-xl font-bold" style={{ color: 'var(--text-primary)' }}>{profileToShow.display_name}</h2>
            ...
            {profileToShow.location && (
              <div className="flex items-center gap-1 mb-2">
                ...
                <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>{profileToShow.location}</p>
              </div>
            )}
            {profileToShow.business_type && (
              <p className="text-sm mb-2" style={{ color: 'var(--text-secondary)' }}>{profileToShow.business_type}</p>
            )}
          </div>
        </div>

        {profileToShow.bio && (
          <p className="text-sm mb-4" style={{ color: 'var(--text-primary)' }}>{profileToShow.bio}</p>
        )}

        <div className="flex items-center gap-6 mb-6">
          <div className="text-center">
            <p className="text-xl font-bold" style={{ color: 'var(--text-primary)' }}>{profileToShow.follower_count}</p>
            <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>Followers</p>
          </div>
          <div className="text-center">
            <p className="text-xl font-bold" style={{ color: 'var(--text-primary)' }}>{profileToShow.following_count}</p>
            <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>Following</p>
          </div>
          <div className="text-center">
            <p className="text-xl font-bold" style={{ color: 'var(--text-primary)' }}>{videos.length}</p>
