...
interface UserRecord {
  ...
  is_verified: boolean; is_admin: boolean; is_banned?: boolean; is_rider?: boolean; created_at: string;
...
export default function AdminDashboardPage() {
  ...
  const loadAllData = async () => {
    ...
    try {
      ...
      const [
        { count: totalUsers }, { count: totalSellers }, { count: totalProducts },
        { count: totalVideos }, { count: verifiedUsers }, { count: newUsersToday },
        { data: videosData }, { data: verificationsData }, { data: adsData }, { data: usersData },
      ] = await Promise.all([
  ...
  const handleVerificationAction = async (id: string, action: 'approve' | 'reject') => {
    setActionLoading(id);
    const updates: Record<string, unknown> = { status: action === 'approve' ? 'approved' : 'rejected', reviewed_at: new Date().toISOString() };
    if (action === 'approve') {
  ...
  const handleUserAction = async (userId: string, action: string) => {
    ...
    if (action === 'make_rider') {
      updates.is_rider = true;
      await supabase.from('riders').upsert({ user_id: userId, is_active: true, is_available: true, vehicle_type: 'bike' }, { onConflict: 'user_id' });
    }
    if (action === 'remove_rider') {
      updates.is_rider = false;
  ...
  const exportCSV = () => {
    const rows = [
      ...
      ['Total Revenue (N)', platformStats.totalRevenue],
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#f1f5f9' }}>
      {toast && (
        <div style={{ position: 'fixed', top: 20, right: 20, zIndex: 9999, padding: '12px 20px', borderRadius: 12, fontWeight: 600, fontSize: 14, backgroundColor: toast.type === 'success' ? '#22c55e' : '#ef4444', color: '#fff', boxShadow: '0 8px 24px rgba(0,0,0,0.2)' }}>
          {toast.msg}
        </div>
      )}
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px', display: 'flex', gap: 4, overflowX: 'auto' }}>
          {([
            ...
            <button key={tab.id} onClick={() => setActiveSection(tab.id)}
              ...
              <tab.icon size={15} />
              {tab.label}
              {tab.badge > 0 && <span style={{ background: '#ef4444', color: '#fff', fontSize: 10, fontWeight: 700, padding: '2px 6px', borderRadius: 10 }}>{tab.badge}</span>}
            </button>
          ))}
        </div>
      </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            <div style={{ background: 'linear-gradient(135deg, #1e40af 0%, #3b82f6 100%)', borderRadius: 20, padding: 28, color: '#fff', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
              <div>
                ...
                <p style={{ fontSize: 40, fontWeight: 900, margin: 0 }}>N{platformStats.totalRevenue.toLocaleString()}</p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
              {[
                ...
                <div key={stat.label} style={{ background: '#fff', borderRadius: 16, padding: 20, border: '1px solid #e2e8f0' }}>
                  <div style={{ width: 44, height: 44, borderRadius: 12, backgroundColor: stat.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 16 }}>
            ...
            {verifications.filter(v => v.status === 'pending' && v.payment_status === 'paid').length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                {verifications.filter(v => v.status === 'pending' && v.payment_status === 'paid').map(v => (
                  <div key={v.id} style={{ background: '#fff', borderRadius: 16, padding: 24, border: '1px solid #e2e8f0' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8, flexWrap: 'wrap', gap: 12 }}>
                          ...
                          <div style={{ textAlign: 'right' }}>
                            <p style={{ fontSize: 22, fontWeight: 800, color: '#22c55e', margin: 0 }}>N{v.amount?.toLocaleString()}</p>
                        ...
                        <div style={{ display: 'flex', gap: 12 }}>
                          <button onClick={() => handleVerificationAction(v.id, 'approve')} disabled={actionLoading === v.id}
                          ...
                          <button onClick={() => handleVerificationAction(v.id, 'reject')} disabled={actionLoading === v.id}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {verifications.filter(v => v.status !== 'pending').slice(0, 10).map(v => (
                    <div key={v.id} style={{ background: '#fff', borderRadius: 12, padding: '16px 20px', border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
                      ...
                      <span style={{ padding: '4px 12px', borderRadius: 20, fontSize: 12, fontWeight: 700, background: v.status === 'approved' ? '#d1fae5' : '#fee2e2', color: v.status === 'approved' ? '#065f46' : '#b91c1c' }}>
        ...
        {activeSection === 'ads' && (
          <div>
            ...
            {ads.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                {ads.map(ad => (
                  <div key={ad.id} style={{ background: '#fff', borderRadius: 16, padding: 24, border: '1px solid #e2e8f0' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16, flexWrap: 'wrap', gap: 12 }}>
                      <div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
                          <h3 style={{ fontSize: 16, fontWeight: 700, color: '#0f172a', margin: 0 }}>{ad.title}</h3>
                          <span style={{ padding: '3px 10px', borderRadius: 20, fontSize: 11, fontWeight: 700, background: ad.status === 'active' ? '#d1fae5' : ad.status === 'pending_payment' ? '#fef3c7' : '#fee2e2', color: ad.status === 'active' ? '#065f46' : ad.status === 'pending_payment' ? '#92400e' : '#b91c1c' }}>
                      ...
                      <div style={{ textAlign: 'right' }}>
                        <p style={{ fontSize: 24, fontWeight: 800, color: '#0f172a', margin: 0 }}>N{ad.budget.toLocaleString()}</p>
                ...
                <div key={u.id} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '14px 20px', borderBottom: i < filteredUsers.length - 1 ? '1px solid #f1f5f9' : 'none' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                      ...
                      {u.is_rider && <span style={{ background: '#fef9c3', color: '#854d0e', fontSize: 10, fontWeight: 700, padding: '2px 6px', borderRadius: 6 }}>RIDER</span>}
                  ...
                  <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                    <button onClick={() => handleUserAction(u.id, u.is_verified ? 'unverify' : 'verify')} disabled={actionLoading === u.id} title={u.is_verified ? 'Remove verification' : 'Verify user'}
                    ...
                    <button onClick={() => handleUserAction(u.id, u.is_admin ? 'remove_admin' : 'make_admin')} disabled={actionLoading === u.id || u.id === user?.id} title={u.is_admin ? 'Remove admin' : 'Make admin'}
                    ...
                    <button onClick={() => handleUserAction(u.id, u.is_banned ? 'unban' : 'ban')} disabled={actionLoading === u.id || u.id === user?.id} title={u.is_banned ? 'Unban user' : 'Ban user'}
                    ...
                    <button onClick={() => handleUserAction(u.id, u.is_rider ? 'remove_rider' : 'make_rider')} disabled={actionLoading === u.id} title={u.is_rider ? 'Remove rider role' : 'Make rider'}
                      style={{ width: 32, height: 32, borderRadius: 8, border: '1px solid #e2e8f0', background: u.is_rider ? '#fef9c3' : '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', opacity: actionLoading === u.id ? 0.5 : 1, fontSize: 14 }}>
                      🛵
                    </button>
                  </div>
                </div>
...
function RidersSection() {
  ...
  useEffect(() => { loadRiders(); loadPendingOrders(); }, []);

  const loadRiders = async () => {
    const { data } = await supabase.from('riders').select('*, user:users!riders_user_id_fkey(id, display_name, avatar_url, phone)').eq('is_active', true).order('created_at', { ascending: false });
    setRiders(data || []);
  };

  const loadPendingOrders = async () => {
    const { data } = await supabase.from('orders').select('*, product:products(title, images), buyer:users!orders_buyer_id_fkey(display_name), seller:users!orders_seller_id_fkey(display_name, location)').in('status', ['paid', 'pending']).is('rider_id', null).order('created_at', { ascending: true });
    setPendingOrders(data || []);
  };

  const assignRider = async (orderId: string, _riderId: string, riderUserId: string) => {
    setAssigning(orderId);
    const eta = new Date(); eta.setMinutes(eta.getMinutes() + 45);
    await supabase.from('orders').update({ rider_id: riderUserId, status: 'assigned', estimated_delivery: eta.toISOString() }).eq('id', orderId);
    await supabase.from('notifications').insert({ user_id: riderUserId, type: 'system', message: 'You have been assigned a new delivery! Open Rider Dashboard to view.' });
    setAssigning(null);
    loadPendingOrders();
  };
  ...
  return (
    <div>
      <h2 style={{ fontSize: 22, fontWeight: 800, color: '#0f172a', marginBottom: 8 }}>Riders & Delivery</h2>
      <p style={{ fontSize: 13, color: '#64748b', marginBottom: 24 }}>Assign riders to paid orders waiting for pickup</p>
      <h3 style={{ fontSize: 16, fontWeight: 700, color: '#0f172a', marginBottom: 12 }}>Active Riders ({riders.length})</h3>
      {riders.length === 0 ? (
        <div style={{ background: '#fff', borderRadius: 16, padding: 32, textAlign: 'center', border: '1px solid #e2e8f0', marginBottom: 24 }}>
          <p style={{ color: '#94a3b8', fontSize: 14 }}>No riders yet. Go to Users tab and assign the rider role to a user.</p>
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 12, marginBottom: 24 }}>
          {riders.map(r => (
            <div key={r.id} style={{ background: '#fff', borderRadius: 16, padding: 16, border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: 12 }}>
              <img src={r.user?.avatar_url || 'https://images.pexels.com/photos/771742/pexels-photo-771742.jpeg?w=200'} style={{ width: 44, height: 44, borderRadius: '50%', objectFit: 'cover' }} alt="" />
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontWeight: 700, fontSize: 14, color: '#0f172a', margin: 0 }}>{r.user?.display_name}</p>
                <p style={{ fontSize: 12, color: '#64748b', margin: '2px 0' }}>🛵 {r.vehicle_type}</p>
                <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 20, background: r.is_available ? '#dcfce7' : '#fee2e2', color: r.is_available ? '#166534' : '#991b1b' }}>
      ...
      <h3 style={{ fontSize: 16, fontWeight: 700, color: '#0f172a', marginBottom: 12 }}>Orders Awaiting Rider ({pendingOrders.length})</h3>
      {pendingOrders.length === 0 ? (
        <div style={{ background: '#fff', borderRadius: 16, padding: 32, textAlign: 'center', border: '1px solid #e2e8f0' }}>
          <p style={{ color: '#94a3b8', fontSize: 14 }}>No orders waiting for assignment</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {pendingOrders.map(order => (
            <div key={order.id} style={{ background: '#fff', borderRadius: 16, padding: 20, border: '1px solid #e2e8f0' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 12 }}>
                <div style={{ width: 48, height: 48, borderRadius: 12, overflow: 'hidden', background: '#f1f5f9', flexShrink: 0 }}>
                  {order.product?.images?.[0] && <img src={order.product.images[0]} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="" />}
                </div>
                <div style={{ flex: 1 }}>
                  <p style={{ fontWeight: 700, fontSize: 14, color: '#0f172a', margin: 0 }}>{order.product?.title || 'Order'}</p>
                  <p style={{ fontSize: 12, color: '#64748b', margin: '2px 0' }}>Buyer: {order.buyer?.display_name}</p>
                  <p style={{ fontSize: 12, color: '#64748b', margin: 0 }}>Seller: {order.seller?.display_name} - {order.seller?.location}</p>
                </div>
                <p style={{ fontWeight: 800, color: '#3b82f6', fontSize: 15, flexShrink: 0 }}>N{Number(order.total_amount).toLocaleString()}</p>
              </div>
              {order.delivery_address && (
                <p style={{ fontSize: 12, color: '#64748b', marginBottom: 12, padding: '8px 12px', background: '#f8fafc', borderRadius: 8 }}>
                  📍 {order.delivery_address}
                </p>
              )}
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {riders.filter(r => r.is_available).map(r => (
                  <button key={r.id} onClick={() => assignRider(order.id, r.id, r.user_id)} disabled={assigning === order.id}
                    style={{ padding: '8px 14px', borderRadius: 10, fontSize: 12, fontWeight: 700, cursor: 'pointer', background: '#0f172a', color: '#fff', border: 'none', opacity: assigning === order.id ? 0.6 : 1 }}>
                    {assigning === order.id ? 'Assigning...' : `Assign to ${r.user?.display_name}`}
                  </button>
                ))}
                {riders.filter(r => r.is_available).length === 0 && (
                  <p style={{ fontSize: 12, color: '#94a3b8' }}>No available riders right now</p>
                )}
              </div>
            </div>
          ))}
