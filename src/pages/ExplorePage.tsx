...
export default function ExplorePage() {
  ...
  // FIX 7: accepts an optional query string so hashtag clicks don't use stale state
  ...
  return (
    <div className="min-h-screen pb-20" style={{ backgroundColor: 'var(--bg-primary)' }}>
      ...
      <div className="p-4">
        {searchResults ? (
          <div>
            <h2 className="text-lg font-bold mb-4" style={{ color: 'var(--text-primary)' }}>Search Results</h2>

            {searchResults.products.length > 0 && (
              <div className="mb-6">
                <h3 className="text-sm font-semibold mb-3" style={{ color: 'var(--text-secondary)' }}>Products</h3>
                <div className="grid grid-cols-2 gap-3">
                  {searchResults.products.map((product) => (
                    <div key={product.id} onClick={() => navigate(`/product/${product.id}`)} className="rounded-xl overflow-hidden cursor-pointer" style={{ backgroundColor: 'var(--bg-secondary)' }}>
                      ...
                      <div className="p-3">
                        <p className="text-sm font-medium line-clamp-1" style={{ color: 'var(--text-primary)' }}>{product.title}</p>
                        <p className="text-lg font-bold mt-1" style={{ color: 'var(--text-primary)' }}>₦{product.price.toLocaleString()}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {searchResults.videos.map((video) => (
                    <div key={video.id} onClick={() => navigate(`/video/${video.id}`)} className="aspect-[9/16] rounded-lg overflow-hidden cursor-pointer" style={{ backgroundColor: 'var(--bg-secondary)' }}>
                      {video.thumbnail_url ? (
                        <img src={video.thumbnail_url} alt="" className="w-full h-full object-cover" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center p-2">
                          <p className="text-xs line-clamp-3" style={{ color: 'var(--text-primary)' }}>{video.caption}</p>
                        </div>
                      )}
                    </div>
                  ))}
            ...
            {searchResults.products.length === 0 && searchResults.videos.length === 0 && (
              <p className="text-center text-sm py-8" style={{ color: 'var(--text-secondary)' }}>No results found</p>
            )}
          </div>
        ) : (
          <div>
            <div className="mb-6">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-lg font-bold" style={{ color: 'var(--text-primary)' }}>Trending Hashtags</h2>
              </div>
              <div className="flex flex-wrap gap-2">
                {trendingHashtags.map((tag) => (
                  <button
                    key={tag}
                    onClick={() => {
                      // FIX 7: pass query directly to avoid stale closure
                      const q = `#${tag}`;
                      setSearchQuery(q);
                      handleSearch(q);
                    }}
            ...
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-lg font-bold" style={{ color: 'var(--text-primary)' }}>Top Products</h2>
              </div>
              <div className="grid grid-cols-2 gap-3">
                {topProducts.map((product) => (
                  <div key={product.id} onClick={() => navigate(`/product/${product.id}`)} className="rounded-xl overflow-hidden cursor-pointer" style={{ backgroundColor: 'var(--bg-secondary)' }}>
                    ...
                    <div className="p-3">
                      <p className="text-sm font-medium line-clamp-1" style={{ color: 'var(--text-primary)' }}>{product.title}</p>
                      <p className="text-xs mb-1" style={{ color: 'var(--text-secondary)' }}>{product.seller?.display_name}</p>
                      <p className="text-lg font-bold" style={{ color: 'var(--text-primary)' }}>₦{product.price.toLocaleString()}</p>
                      <p className="text-xs mt-1" style={{ color: 'var(--text-tertiary)' }}>{product.purchase_count} sold</p>
                    </div>
                  </div>
                ))}
              </div>
