...
export default function AdminDashboardPage() {
  ...
  const handleUserAction = async (userId: string, action: 'ban' | 'unban' | 'make_admin' | 'remove_admin' | 'verify' | 'unverify' | 'make_rider' | 'remove_rider') => {
                  ...
                  <div key={u.id} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '14px 20px', borderBottom: i < filteredUsers.length - 1 ? '1px solid #f1f5f9' : 'none' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                        ...
                        {(u as any).is_rider && <span style={{ background: '#fef9c3', color: '#854d0e', fontSize: 10, fontWeight: 700, padding: '2px 6px', borderRadius: 6 }}>🛵 RIDER</span>}
                    ...
                    <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                      ...
                      <button
                        onClick={() => handleUserAction(u.id, (u as any).is_rider ? 'remove_rider' : 'make_rider')}
                        disabled={actionLoading === u.id}
                        title={(u as any).is_rider ? 'Remove rider role' : 'Make rider 🛵'}
                        style={{ width: 32, height: 32, borderRadius: 8, border: '1px solid #e2e8f0', background: (u as any).is_rider ? '#fef9c3' : '#f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', opacity: actionLoading === u.id ? 0.5 : 1, fontSize: 14 }}
                      >
                        🛵
                      </button>
                    </div>
...
// ─── Riders & Order Assignment Section ───────────────────────────────────────
function RidersSection() {
  ...
  const loadRiders = async () => {
    const { data } = await supabase
      .from('riders')
      .select('*, user:users!riders_user_id_fkey(id, display_name, avatar_url, phone)')
      .eq('is_active', true)
  ...
  const loadPendingOrders = async () => {
    const { data } = await supabase
      .from('orders')
      .select('*, product:products(title, images), buyer:users!orders_buyer_id_fkey(display_name), seller:users!orders_seller_id_fkey(display_name, location)')
  ...
  const assignRider = async (orderId: string, riderId: string, riderUserId: string) => {
    ...
    await supabase.from('notifications').insert({
      user_id: riderUserId, type: 'system',
      message: `You have been assigned a new delivery! Open Rider Dashboard to view.`
    });
  ...
  return (
    <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 12, marginBottom: 24 }}>
          {riders.map(r => (
            <div key={r.id} style={{ background: '#fff', borderRadius: 16, padding: 16, border: '1px solid #e2e8f0', display: 'flex', alignItems: 'center', gap: 12 }}>
              <img src={r.user?.avatar_url || 'https://images.pexels.com/photos/771742/pexels-photo-771742.jpeg?w=200'}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 20,
                  background: r.is_available ? '#dcfce7' : '#fee2e2',
      ...
      <h3 style={{ fontSize: 16, fontWeight: 700, color: '#0f172a', marginBottom: 12 }}>
        Orders Awaiting Rider ({pendingOrders.length})
      </h3>
            ...
            <div key={order.id} style={{ background: '#fff', borderRadius: 16, padding: 20, border: '1px solid #e2e8f0' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 12 }}>
                <div style={{ width: 48, height: 48, borderRadius: 12, overflow: 'hidden', background: '#f1f5f9', flexShrink: 0 }}>
                  {order.product?.images?.[0] && <img src={order.product.images[0]} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />}
                </div>
                <div style={{ flex: 1 }}>
                ...
                <p style={{ fontWeight: 800, color: '#3b82f6', fontSize: 15, flexShrink: 0 }}>₦{Number(order.total_amount).toLocaleString()}</p>
              </div>
