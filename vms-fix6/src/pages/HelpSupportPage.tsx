...
function ChatWindow({ onClose }: { onClose: () => void }) {
  ...
  const initSession = async () => {
    ...
    // Subscribe to realtime updates — return channel so useEffect can clean up
  ...
  const escalateToAgent = async () => {
    if (!session) return;
    await supabase.from('support_sessions').update({ status: 'waiting' }).eq('id', session.id);
    setSession(prev => prev ? { ...prev, status: 'waiting' } : prev);
    await addSystemMessage(session.id, '⏳ You\'ve been connected to our support queue. An agent will join shortly. Average wait time: 2-5 minutes.\n\nYou can keep chatting — the agent will see the full conversation history.');
  };
  ...
  const sendMessage = async () => {
    ...
    // OPTIMISTIC: show message immediately before DB write
    const optimisticMsg = {
    ...
    const { data: savedMsg } = await supabase.from('support_messages').insert({
      session_id: session.id,
      sender_type: 'user',
      sender_id: user?.id,
      content: text,
    ...
    if (savedMsg) {
      setMessages(prev => prev.map(m => m.id === optimisticMsg.id ? savedMsg as any : m));
    }
    ...
    if (session.status === 'bot') {
      ...
      // Check for "yes" escalation confirmation
      ...
      {session?.status !== 'resolved' && session?.status !== 'closed' && (
        <div style={{
          ...
          <input
            ...
            placeholder={session?.status === 'waiting' ? 'Agent will join soon...' : 'Type your message...'}
            style={{
